...
\begin{frame}
	...
	\end{figure}
	\vspace{-2em}
	\(\rightarrow\) chain of states \(\gamma : c_0\to c_1\to c_2\to\ldots\quad\) at times \(t_0,t_1,t_2,\ldots\)
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Shannon entropy is the mean system entropy, i.e. the average over all configurations:
			\Align{
				\langle S_{\text{System}}(t) \rangle = \sum_c -P_{c(t)} (t)\ln P_{c(t)} (t)
			}
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	However, this leads to momenta pointing in the other direction as velocities.
\end{frame}
...
\begin{frame}
	...
	\begin{figure}[htb]
		\vspace{-1em}
		\includegraphics[width=15em]{reversal.png}
		\vspace{-1em}
	\end{figure}
	\begin{itemize}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item is not unique, represented by the index \(a\)
...
\begin{frame}
	...
	Using this approach, we showed that the Spinney/Ford approach for the simplest case of \(\gamma(p),\Gamma(p)=const\) yields
	\Align{
		\mathrm dS_\text{env} = -\beta p\mathrm dp - \gamma \mathrm dt
	}
	This seems very implausible, as a resting particle (\(p=0\)) produces negative entropy over time.
\end{frame}
...
\begin{frame}
	...
	\begin{figure}[htb]
		\vspace{-1em}
		\includegraphics[width=18em]{trajectories.png}
		\vspace{-1em}
	\end{figure}
	\begin{itemize}
